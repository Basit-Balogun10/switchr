...
export function AuthForm({ onClose }: AuthFormProps) {
  ...
  const validateForm = (): string | null => {
    ...
    if (isSignUp) {
      if (selectedUserType === "user" && (!formData.fullName || formData.fullName.trim().length < 2)) {
        return "Please enter your full name";
      }
      if (selectedUserType === "provider" && (!formData.companyName || formData.companyName.trim().length < 2)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        signInData.phone = formData.phone;
